...
#define MATERIAL_IMAGE_PATH "/dev_hdd0/tmp/bonk/images/"
	...
	if (begin_window("materials"))
	{
		...
		if (begin_panel("#materials"))
		{
			...
			for (int i = 0; i < material_count; i++)
			{
				...
				if (strstr(material->name, g_searchStr.data()))
				{
					submenu(va("%s", material->name), "selected material", [=]() { g_curMaterial = (Material*)*(int*)(0x10040000 + (i * 4)); });
				}
			}
			end_panel();
		}
		...
		{
			label("print address", []() {
				printf("name: %s\naddress: 0x%X", g_curMaterial->name, g_curMaterial);
				g_log.print("name: %s address: 0x%X\n", g_curMaterial->name, g_curMaterial);
			});
			divider(va("texture count: %i", g_curMaterial->get_texture_count()));
			divider(va("constant count: %i", g_curMaterial->get_constant_count()));
		...
		{
			for (int i = 0; i < g_curMaterial->get_texture_count(); i++)
			{
		...
		{
			for (int i = 0; i < g_curMaterial->get_constant_count(); i++)
			{
		...
		if (begin_subpanel("selected texture"))
		{
			...
			submenu("injector", "image injector", []() {
				image_list = libpsutil::filesystem::list_files(MATERIAL_IMAGE_PATH, true);
			});
		...
		if (begin_subpanel("selected constant"))
		{
			...
			{
				colorpicker("color", *(color*)&g_curConstant->vVec4);
			}
			else
			{
				divider("divider");
				slider("tweak x", g_curConstant->vVec4.x, 1.f, 0.f, 1000.f);
				slider("tweak y", g_curConstant->vVec4.y, 1.f, 0.f, 1000.f);
				slider("tweak z", g_curConstant->vVec4.w, 1.f, 0.f, 1000.f);
				slider("tweak w", g_curConstant->vVec4.z, 1.f, 0.f, 1000.f);
			}
			end_subpanel();
		}
